...
def _build_individual_cells(activity: Activity, all_groups: List[str], activity_color: str, text_color: str, with_tone_class) -> List[Dict[str, Any]]:
    ...
    # Individuals bleiben immer je Positionsgruppe sichtbar. Auch wenn noch
    # keine einzelnen Themen erfasst sind, bleibt die Unterteilung erhalten.
    fallback_topic = activity.topic or ''
    return [{
        'colspan': 1,
        'groups': [group],
        'class': with_tone_class('table-success', [group]),
        'content': _topic_for(group) or fallback_topic,
        'color': activity_color,
        'text_color': text_color,
