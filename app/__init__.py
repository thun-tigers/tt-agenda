...
from .routes import main, auth, admin, api, users
...
def create_app(config_class=Config):
    ...
    if (app.config.get('AUTO_CREATE_DB') and app.config.get('CREATE_DEFAULT_USERS', True)
            and not app.config.get('DEFAULT_ADMIN_PASSWORD') and not app.testing):
        raise RuntimeError('DEFAULT_ADMIN_PASSWORD must be set when default users are enabled.')
    ...
    def inject_platform_links():
        ...
        return {
            'auth_dashboard_url': url_for('main.index'),
...
def seed_default_admin(app):
    if User.query.count() or not app.config.get('CREATE_DEFAULT_USERS', True):
        return
    admin = User(
        ...
        first_name='Tigers',
        last_name='Admin',
        display_name='Tigers Admin',
        profile_complete=True,
        memberships_json=[{'team_code': 'SENIORS', 'team_name': 'Seniors', 'member_role': 'coach'}],
    )
    password = app.config.get('DEFAULT_ADMIN_PASSWORD')
    if not password:
        raise RuntimeError('DEFAULT_ADMIN_PASSWORD must be set before creating the default admin.')
