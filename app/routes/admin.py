...
def _load_admin_training_lists(query_text, type_filter, include_ended):
    """Load the three admin lists with one shared filtering implementation."""
    today = date.today()
    team_code = get_active_team_code()

    def apply_name_filter(query, model):
        if query_text:
            query = query.filter(model.name.ilike(f'%{query_text}%'))
        return query

    trainings_query = apply_name_filter(_scoped_training_query(is_hidden=False), Training)
    if not include_ended:
        trainings_query = trainings_query.filter(Training.end_date >= today)
    trainings = (trainings_query.order_by(Training.start_date.desc()).all()
                 if type_filter in ['all', 'template'] else [])

    hidden_query = apply_name_filter(_scoped_training_query(is_hidden=True), Training)
    if not include_ended:
        hidden_query = hidden_query.filter(Training.start_date >= today)
    hidden_trainings = (hidden_query.order_by(Training.start_date.desc()).all()
                        if type_filter in ['all', 'hidden'] else [])

    instances_query = TrainingInstance.query.join(Training).filter(Training.team_code == team_code)
    if query_text:
        instances_query = instances_query.filter(Training.name.ilike(f'%{query_text}%'))
    if not include_ended:
        instances_query = instances_query.filter(TrainingInstance.date >= today)
    instances = (instances_query.order_by(TrainingInstance.date.desc()).all()
                 if type_filter in ['all', 'instance'] else [])
    return trainings, hidden_trainings, instances
...
def admin_trainings():
    ...
    trainings, hidden_trainings, instances = _load_admin_training_lists(q, type_filter, include_ended)
...
def admin_agenda_categories():
    ...
    if request.method == 'POST':
        ...
        db.session.commit()
        flash('Agenda-Kategorien aktualisiert.', 'success')
